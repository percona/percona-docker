...
RUN set -ex; \
    dnf -y install \
        ...
        policycoreutils; \
        \
    curl -Lf -o /tmp/Percona-Server-MongoDB-server.rpm http://repo.percona.com/psmdb-60/yum/release/8/RPMS/x86_64/percona-server-mongodb-server-${FULL_PERCONA_VERSION}.x86_64.rpm; \
...
RUN useradd -u 1001 -r -g mongod -m -s /sbin/nologin \
